...
class Game {
  constructor(initialState) {
    this.initialState = initialState ? this.copyState(initialState) : this.emptyBoard();
  ...
  moveRight() {
    if (this.status !== 'playing') return;
    const newState = this.state.map(row => this.compressAndMerge([...row].reverse()).reverse());
    if (!this.equals(newState, this.state)) {
      this.state = newState;
      this.addRandomTile();
    }
  ...
  moveDown() {
    if (this.status !== 'playing') return;
    const transposed = this.transpose(this.state);
    const newTransposed = transposed.map(col => this.compressAndMerge([...col].reverse()).reverse());
    const newState = this.transpose(newTransposed);
    if (!this.equals(newState, this.state)) {
      this.state = newState;
      this.addRandomTile();
